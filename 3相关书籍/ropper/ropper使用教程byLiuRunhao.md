...
建议python3安装，比python2方便许多。

推荐一个帖子[Ubuntu下安装Ropper的坑](https://blog.csdn.net/saislanti/article/details/102520233)

里面介绍了一些安装的方法，但每台机器不一样，还需要根据自己的情况解决一些小问题。

想要安装所有的依赖要研究一下。

## 使用教程

ropper是一个效率比较高的gadget查找工具，而且跨平台`x86/X86_64, ARM/ARM64, MIPS/MIPS64, PowerPC/PowerPC64, SPARC64`，支持多种文件格式`ELF、PE、Mach-O、Raw`。
...
```shell
iddm@ubuntu:~⟫ ropper -f ~/Desktop/libc.so.6 --semantic rax==0
...
## 我的依赖库安装的有一些问题，所以导致没有查到
```

也可以通过交互来做：

```shell
148 iddm@ubuntu:~⟫ ropper
(ropper)> file /home/iddm/Desktop/libc.so.6
[INFO] Load gadgets from cache
[LOAD] loading... 100%
[LOAD] removing double gadgets... 100%
[INFO] File loaded.
(libc.so.6/ELF/x86_64)> semantic rax==0
[INFO] Searching for gadgets: rax==0
[INFO] 0 gadgets found
...
下面我就贴一下ropper的github上面的readme，起初找了找ropper使用教程没有找到，后面打算自己写一篇方便别人，现在弄清楚之后发现github上面的readme写的很清楚了，我就对readme加一些注释方便理解。
...
supported filetypes:#支持的文件格式
  ELF
  PE
  Mach-O
  Raw

supported architectures:#支持的框架
  x86 [x86]
  x86_64 [x86_64]
  MIPS [MIPS, MIPS64]
...
available rop chain generators:#甚至可以直接帮助构造rop chain
...
optional arguments:
  ...
  --console             Starts interactive commandline #开启交互窗口
  ## ropper -f file_path --console
  -f <file>, --file <file>
                        The file to load
  -r, --raw             Loads the file as raw file
  -a <arch>, --arch <arch> #制定框架类型
 ...
 #set option 设置选项，有几个默认的选项（见scripts）
  ...
  ## Searches for 'pop reg; pop reg; ret'
  ...
  --chain <generator>   Generates a ropchain [generator parameter=value[
                        parameter=value]]
  ##设置badtytes  保证badtypes制定的值不会出现在gadget中
...
rs.setArchitectureFor(name=ls, arch='x86') #可以帮助他指定一下框架
...
一个很好用的gadget查找工具，非常方便使用，try it and you will love it.
